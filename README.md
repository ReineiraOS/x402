...
> **Status: testnet only, work in progress.** This repository is a scaffold. One contract, the
> `X402EscrowReceiver` funding source in `packages/contracts`, is intentionally a stub (marked in
> code) pending implementation. Nothing here is a production financial product. See
...
- **On-chain layer is non-custodial demo/conformance code.** No value-moving primitives are deployed
  today — the `X402EscrowReceiver` funding source is a stub and real settlement is unimplemented.
  Nothing here implies funds are escrowed on-chain now.
