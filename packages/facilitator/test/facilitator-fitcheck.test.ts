...
function buildFacilitator() {
  ...
  const signer = toFacilitatorEvmSigner(Object.assign(wallet, { address: account.address }));
...
describe("x402 v2 facilitator reuse fit-check (Arbitrum Sepolia)", () => {
  it("registerExactEvmScheme wires eip155:421614 into getSupported", () => {
    ...
    expect(fresh.getSupported().kinds.some((k) => k.network === "eip155:421614")).toBe(false);

    const facilitator = buildFacilitator();
    const supported = facilitator.getSupported();
    expect(supported.kinds.some((k) => k.network === "eip155:421614" && k.scheme === "exact")).toBe(
      true,
    );
  });
});
