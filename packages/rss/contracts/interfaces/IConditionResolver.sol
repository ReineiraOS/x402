...
/// @dev Implement this interface to plug a custom condition into the escrow protocol.
///      The escrow contract calls `onConditionSet` during `create`, `getConditionFee`
///      during `_setCondition`, and `isConditionMet` before allowing redemption.
///      Implementations must support ERC-165 so callers can validate the interface.
interface IConditionResolver is IERC165 {
    ...
    /// @notice Called by the escrow contract when a condition is attached to a new escrow
    /// @param escrowId The escrow identifier
    /// @param data Resolver-specific configuration data forwarded from `Escrow.create`
    function onConditionSet(uint256 escrowId, bytes calldata data) external;

    /// @notice Returns the condition fee that should be stamped on the escrow
    /// @dev Called once at stamp time. Returned `bps` is bounded by `MAX_TOTAL_BPS = 10000`
    ///      via the sum invariant in `Escrow._stampFee`. There is no per-fee on-chain cap;
    ///      sanity is expected to be enforced by the calling UI.
