...
export function RiskCallout({ tier }: { tier: TrustTier }) {
  ...
  return (
    <div
      ...
      style={{
        borderRadius: "var(--r-sub)",
        borderLeft: "3px solid var(--st-spec)",
        background: "rgba(243,166,40,.08)",
        padding: "13px 16px",
      }}
    >
      <div
      ...
      <div style={{ fontSize: 14, lineHeight: 1.6, color: "var(--text-secondary)" }}>
        {word} is unaudited and testnet-only. Interacting with deployed contracts may result in loss
        of testnet funds — use at your own risk. Inspect the source before integrating.
      </div>
    </div>
  );
}
