...
const wallet = (acct: typeof backend) =>
  createWalletClient({ account: acct, chain: arbitrumSepolia, transport: http(RPC) });
...
async function main() {
  ...
  assert(
    await pub.readContract({
      ...
      args: [PAUSER_ROLE, guardian.address],
    }),
    "Guardian holds PAUSER_ROLE",
  );
  assert(
    await pub.readContract({
      address: VAULT,
      abi: vaultAbi,
      functionName: "hasRole",
      args: [DEMO_ROLE, backend.address],
    }),
    "Backend holds DEMO_ROLE",
  );
  assert(
    !(await pub.readContract({
      address: VAULT,
      abi: vaultAbi,
      functionName: "hasRole",
      args: [PAUSER_ROLE, sentinel.address],
    })),
    "Sentinel does NOT hold PAUSER_ROLE (two-key separation)",
  );
  // ensure healthy at start (restore from any prior run)
  const ta0 = (await pub.readContract({
    address: VAULT,
    abi: vaultAbi,
    functionName: "totalAssets",
  })) as bigint;
  const fl0 = (await pub.readContract({
    address: VAULT,
    abi: vaultAbi,
  ...
  assert(
    await pub.readContract({ address: VAULT, abi: vaultAbi, functionName: "isHealthy" }),
    "vault healthy at start",
  );

  console.log("\nSTEP 1 — Sentinel stakes a bond into an AlertResolver-gated escrow");
  const created = await send(backend, {
    address: ESCROW,
    abi: escrowAbi,
    functionName: "create",
    args: [
      ...
      BOND,
      RESOLVER,
      encodeAbiParameters([{ type: "address" }], [sentinel.address]),
    ],
  });
  ...
  assert(
    ...
    "resolver snapshotted vault floor",
  );
  assert(
    !(await pub.readContract({
      address: RESOLVER,
    ...
    "verdict false while vault healthy (false alarm would lock bond)",
  );
  ...
  const funded = await send(sentinel, {
    address: ESCROW,
    abi: escrowAbi,
    functionName: "fund",
    args: [escrowId, BOND],
  ...
  assert(
    !(await pub.readContract({ address: VAULT, abi: vaultAbi, functionName: "isHealthy" })),
    "vault invariant broken (totalAssets < floor)",
  );

  console.log("\nSTEP 3 — Guardian (distinct key) pauses the vault, freezing further damage");
  const paused = await send(guardian, {
    address: VAULT,
    abi: vaultAbi,
    functionName: "pause",
    args: [],
  ...
  assert(
    await pub.readContract({ address: VAULT, abi: vaultAbi, functionName: "paused" }),
    "vault PAUSED · funds safe",
  );
  ...
  assert(
    ...
    "AlertResolver.isBreached == true (VALID)",
  );
  assert(
    await pub.readContract({
      address: RESOLVER,
    ...
    "isConditionMet == true → bond redeemable",
  );
  ...
  const balBefore = (await pub.readContract({
    ...
    args: [sentinel.address],
  })) as bigint;
  const redeemed = await send(sentinel, {
    address: ESCROW,
    abi: escrowAbi,
    functionName: "redeem",
    args: [escrowId],
  ...
  assert(
    balAfter > balBefore,
    `Sentinel got the bond back (+${Number(balAfter - balBefore) / 1e6} USDC)`,
  );
  ...
  console.log(
    JSON.stringify(
      {
        bondCreate: created.hash,
        bondFund: funded.hash,
        guardianPause: paused.hash,
        stagedDrain: drained.hash,
        bondRedeem: redeemed.hash,
