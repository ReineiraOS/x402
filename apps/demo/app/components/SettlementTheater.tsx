      ...
      <div className="flow bw-card">
        ...
        <div className="flow__status">
          {locked ? (
            ...
            <div className="flow__count flow__count--idle">
              <Icon name="play" size={13} stroke={2} />
              <span className="mono">awaiting run</span>
              <span className="flow__bar" aria-hidden>
                <span style={{ width: "0%" }} />
              </span>
            </div>
        ...
        {tab === "console" ? (
          <div className="term__foot">
            ...
            <div className={`an-dd an-dd--up an-dd--block${pickOpen ? " open" : ""}`} ref={pickRef}>
              ...
              >
                {selectedResource
                  ? `${selectedResource.name} · ${usdc(selectedResource.priceAtomic)}`
                  : "select a resource"}
              </button>
              <div className="an-dd__menu" role="listbox">
                {resources.map((r) => (
                  <div
                    key={r.id}
                    className={`an-dd__item${resourceId === r.id ? " active" : ""}`}
                    role="option"
                    aria-selected={resourceId === r.id}
                    onClick={() => {
                      setResourceId(r.id);
                      setPickOpen(false);
                    }}
