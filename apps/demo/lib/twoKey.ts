...
async function stakeBondOverX402(
  ...
  try {
    ...
    const payment = {
      ...
      resource: {
        url: "/api/resource",
        description: "Vault security bond",
    ...
    await ctx.send(ctx.sentinel, {
      ...
      args: [cfg.escrow, BOND_ATOMIC],
    });
    const funded = await ctx.send(ctx.sentinel, {
      address: cfg.escrow,
      abi: escrowAbi,
      functionName: "fund",
      args: [escrowId, BOND_ATOMIC],
    });
...
export async function runTwoKeyHalt(args: {
  ...
  if (!cfg)
    throw new Error(
      "Two-Key Halt is not configured (VAULT/ALERT_RESOLVER/GUARDIAN/SENTINEL env missing)",
    );
  ...
  if (v.totalAssets < v.floor)
    await ctx.send(ctx.backend, {
      address: cfg.vault,
      abi: vaultAbi,
      functionName: "deposit",
      args: [v.floor - v.totalAssets],
    });
  ...
  if (sentinelUsdc < BOND_ATOMIC) {
    await ctx.send(ctx.backend, {
      ...
      args: [ctx.sentinel.address, BOND_ATOMIC * 3n - sentinelUsdc],
    });
  }
  ...
  emit({
    ...
    totalAssets: v.totalAssets.toString(),
    recordedFloor: v.floor.toString(),
    msg: "Vault healthy · balance above its floor",
  });
  emit({
    zone: "system",
    msg: "One vault, two independent agents: a Sentinel that can raise an alarm, a Guardian that can freeze. Neither can do the other's job.",
  });
  await sleep(STEP_MS);
  ...
  emit({
    zone: "sentinel",
    msg: "Sentinel watching the vault — to raise an alarm it must stake a bond.",
  });
  await sleep(STEP_MS);
  const created = await ctx.send(ctx.backend, {
    ...
    functionName: "create",
    args: [
      ctx.sentinel.address,
      BOND_ATOMIC,
      cfg.resolver,
      encodeAbiParameters([{ type: "address" }], [ctx.sentinel.address]),
    ],
  });
  ...
  if (!forceFalseAlarm) {
    ...
    emit({
      zone: "system",
      label: "staged",
      msg: "Staged attacker transaction — drains the vault below its recorded floor. (Labelled staged, like mocked compute.)",
    });
    ...
    emit({
      ...
      totalAssets: v.totalAssets.toString(),
      recordedFloor: v.floor.toString(),
      msg: "balance fell below the floor — the breach is real",
      tx: drained.hash,
    ...
    const latched = await ctx.send(ctx.sentinel, {
    ...
    emit({
      ...
      label: "scripted",
      kind: "alert",
      msg: "Sentinel raises the alarm and commits the breach on-chain. The decision to alarm is scripted; the on-chain proof is real.",
      tx: latched.hash,
      arbiscan: arbiscan(latched.hash),
    });
    await sleep(STEP_MS);
  } else {
    emit({
      ...
      label: "scripted",
      kind: "alert",
      msg: "Sentinel raises an alarm — but the vault is actually fine. A false alarm.",
    });
    await sleep(STEP_MS);
  }

  // STEP 3 — the Guardian (distinct key, can pause but not move funds) freezes the vault.
  const paused = await ctx.send(ctx.guardian, {
    address: cfg.vault,
    abi: vaultAbi,
    functionName: "pause",
  });
  ...
  emit({
    zone: "guardian",
    kind: "paused",
    msg: "Guardian sees the bonded alert and pauses the vault — it never spoke to the Sentinel, it trusted the bond.",
    tx: paused.hash,
    arbiscan: arbiscan(paused.hash),
  });
  emit({
    zone: "vault",
    kind: "vault",
    ...
    totalAssets: v.totalAssets.toString(),
    recordedFloor: v.floor.toString(),
    msg: "frozen — the Guardian blocks any further drain",
  });
  await sleep(STEP_MS);

  // STEP 4 — trustless verdict: the resolver reads the real on-chain flag.
  const valid = await ctx.read<boolean>({
    address: cfg.resolver,
    abi: resolverAbi,
  ...
  if (valid && !forceFalseAlarm) {
    emit({
      ...
      kind: "verdict",
      status: "VALID",
      msg: "Verdict VALID — the resolver, not an operator, confirms the on-chain floor really broke.",
    });
    await sleep(STEP_MS);
    ...
    emit({
      ...
      kind: "vault",
      state: "safe",
      msg: "frozen — drain stopped; the rest of the vault is safe",
    });
    ...
    emit({
      zone: "system",
      label: "ledger",
      msg: "Ledger — LIVE: bond stake, Guardian pause, on-chain verdict, bond return. STAGED: attacker tx + detection. DEFERRED: funded bounty pool.",
    });
  ...
  emit({
    ...
    kind: "verdict",
    status: "FALSE",
    msg: "Verdict FALSE — the resolver reads the vault as healthy. The alarm was wrong.",
  });
  await sleep(STEP_MS);
  ...
  emit({
    zone: "system",
    label: "ledger",
    msg: "Ledger — LIVE: bond stake, Guardian pause/unpause, on-chain verdict. STAGED: detection. The false alarm forfeited the bond.",
  });
