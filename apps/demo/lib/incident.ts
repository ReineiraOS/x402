...
export async function runIncidentResponse(args: {
  ...
  const total = await ctx.read<bigint>({ address: cfg.vault, abi: vaultAbi, functionName: "totalAssets" });
  ...
  if (sentinelUsdc < maxBond) await ctx.send(ctx.backend, { address: cfg.usdc, abi: erc20Abi, functionName: "transfer", args: [ctx.sentinel.address, maxBond * 3n - sentinelUsdc] });
  ...
  if (ready) {
    ...
    emit({ zone: "incident", kind: "coverage", msg: cov.coverageId !== null ? `Depositor insured for ${fmt(payoutAtomic)} (coverage #${cov.coverageId}).` : `Coverage attach failed: ${cov.note}`, tx: cov.tx ?? undefined, arbiscan: cov.tx ? arbiscan(cov.tx) : undefined });
  } else {
