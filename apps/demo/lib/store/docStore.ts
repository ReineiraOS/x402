...
// A single JSON document persisted either as a local file (zero-config dev) or as one row
// in Postgres `kv_store` (Vercel/Neon). The same-instance lock serializes read-modify-write
// within a process; cross-instance writes are last-write-wins, which is acceptable for the
// testnet demo's sequential, single-operator flow.
export function createDocStore<T>(opts: {
  ...
  if (storageBackend === "file") {
    const file = path.join(process.cwd(), opts.fileName);
    return {
      withLock,
      async read() {
        try {
  ...
  return {
    ...
    async read() {
      ...
      const rows = (await db`SELECT value FROM kv_store WHERE key = ${opts.pgKey}`) as {
        value: T;
      }[];
    ...
    async write(value) {
      ...
      await db`
        ...
        VALUES (${opts.pgKey}, ${JSON.stringify(value)}::jsonb, now())
