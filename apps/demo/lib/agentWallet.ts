...
const DEFAULT_ZERODEV_PROJECT_ID = "866d15a6-e621-4e6a-b796-634611f34211";
...
function zerodevRpc(projectId: string): string {
...
export async function createAgentWallet(
  ...
  const paymasterClient = createZeroDevPaymasterClient({
    chain: arbitrumSepolia,
    transport: http(zerodevRpc(projectId)),
  });

  const kernelClient = createKernelAccountClient({
    account,
    chain: arbitrumSepolia,
    bundlerTransport: http(zerodevRpc(projectId)),
    client: publicClient,
